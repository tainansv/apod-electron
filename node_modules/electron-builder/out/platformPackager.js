...
class PlatformPackager {
    constructor(info) {
        this.info = info;
        this._resourceList = new (_electronBuilderUtil || _load_electronBuilderUtil()).Lazy(() => (0, (_promise || _load_promise()).orIfFileNotExist)((0, (_fsExtraP || _load_fsExtraP()).readdir)(this.buildResourcesDir), []));
    ...
    computeAppOutDir(outDir, arch) {
        return this.info.prepackaged || _path.join(outDir, `${this.platform.buildConfigurationKey}${(0, (_core || _load_core()).getArchSuffix)(arch)}${this.platform === (_core || _load_core()).Platform.MAC ? "" : "-unpacked"}`);
    }
    ...
    doPack(outDir, appOutDir, platformName, arch, platformSpecificBuildOptions, targets) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const macroExpander = function (it) {
                return _this2.expandMacro(it, arch == null ? null : (_core || _load_core()).Arch[arch], { "/*": "{,/**/*}" });
            };
            ...
            (0, (_log || _load_log()).log)(`Packaging for ${platformName} ${(_core || _load_core()).Arch[arch]} using ${isElectron ? `electron ${_this2.info.electronVersion}` : `muon ${muonVersion}`} to ${_path.relative(_this2.projectDir, appOutDir)}`);
            const appDir = _this2.info.appDir;
            const ignoreFiles = new Set([_path.resolve(_this2.info.projectDir, outDir), _path.resolve(_this2.info.projectDir, "electron-builder.yml"), _path.resolve(_this2.info.projectDir, "electron-builder.json"), _path.resolve(_this2.info.projectDir, "electron-builder.json5")]);
            if (_this2.info.isPrepackedAppAsar) {
                yield (0, (_dirPackager || _load_dirPackager()).unpackElectron)(_this2, appOutDir, platformName, (_core || _load_core()).Arch[arch], _this2.info.electronVersion);
            } else {
                yield isElectron ? (0, (_dirPackager || _load_dirPackager()).unpackElectron)(_this2, appOutDir, platformName, (_core || _load_core()).Arch[arch], _this2.info.electronVersion) : (0, (_dirPackager || _load_dirPackager()).unpackMuon)(_this2, appOutDir, platformName, (_core || _load_core()).Arch[arch], muonVersion);
                if (asarOptions == null) {
                    yield (0, (_packageDependencies || _load_packageDependencies()).dependencies)(appDir, ignoreFiles);
                }
            }
            let rawFilter = null;
            ...
            const defaultMatcher = (0, (_fileMatcher || _load_fileMatcher()).createFileMatcher)(_this2.info, appDir, resourcesPath, macroExpander, platformSpecificBuildOptions, _path.resolve(_this2.info.projectDir, _this2.buildResourcesDir));
            ...
            const transformer = yield (0, (_fileTransformer || _load_fileTransformer()).createTransformer)(appDir, isElectronCompile ? Object.assign({
                ...
                main: (_asarUtil || _load_asarUtil()).ELECTRON_COMPILE_SHIM_FILENAME
            }, _this2.config.extraMetadata) : _this2.config.extraMetadata);
            let promise;
            if (_this2.info.isPrepackedAppAsar) {
                ...
                promise = new (_asarUtil || _load_asarUtil()).AsarPackager(appDir, resourcesPath, asarOptions, fileMatcher == null ? null : fileMatcher.createFilter(), transformer).pack(filter, isElectronCompile, _this2);
            }
            ...
            if (platformName === "darwin" || platformName === "mas") {
                yield require("./packager/mac").createApp(_this2, appOutDir, asarOptions == null ? null : yield (0, (_asarIntegrity || _load_asarIntegrity()).computeData)(resourcesPath, asarOptions.externalAllowed ? { externalAllowed: true } : null));
            }
    ...
    computeSafeArtifactName(ext, arch) {
        let skipArchIfX64 = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : true;

        return this.computeArtifactName("${name}-${version}-${arch}.${ext}", ext, skipArchIfX64 && arch === (_core || _load_core()).Arch.x64 ? null : arch);
    }
    expandArtifactNamePattern(targetSpecificOptions, ext, arch, defaultPattern) {
        ...
        return this.computeArtifactName(pattern, ext, skipArchIfX64 && arch === (_core || _load_core()).Arch.x64 ? null : arch);
    }
    computeArtifactName(pattern, ext, arch) {
        let archName = arch == null ? null : (_core || _load_core()).Arch[arch];
        if (arch === (_core || _load_core()).Arch.x64) {
            if (ext === "AppImage" || ext === "rpm") {
                archName = "x86_64";
            } else if (ext === "deb") {
                archName = "amd64";
            }
        } else if (arch === (_core || _load_core()).Arch.ia32) {
    ...
    generateName(ext, arch, deployment) {
        ...
        } else if (arch === (_core || _load_core()).Arch.ia32 && ext === "deb") {
            c = "i386";
        } else if (ext === "pacman") {
            if (arch === (_core || _load_core()).Arch.ia32) {
                c = "i686";
            }
            e = "pkg.tar.xz";
        } else {
            c = (_core || _load_core()).Arch[arch];
        }
        if (c == null) {
            c = classifier;
