...
class SnapTarget extends (_core || _load_core()).Target {
    ...
    build(appOutDir, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            (0, (_log || _load_log()).log)(`Building Snap for arch ${(_core || _load_core()).Arch[arch]}`);
            ...
            const after = (0, (_electronBuilderUtil || _load_electronBuilderUtil()).replaceDefault)(options.after, defaultAfter);
            snap.parts = {
                app: {
                    plugin: "dump",
            ...
            const snapFileName = `${snap.name}_${snap.version}_${(0, (_core || _load_core()).toLinuxArchString)(arch)}.snap`;
            ...
            if (isUseDocker) {
                ...
                "-v", `${_this.outDir}:/out`, "electronuserland/electron-builder:latest", "/bin/bash", "-c", `${mayInstallBuildPackagesCmd()} && snapcraft --version && cp -R /out/${_path.basename(stageDir)} /s/ && cd /s && snapcraft snap --target-arch ${(0, (_core || _load_core()).toLinuxArchString)(arch)} -o /out/${snapFileName}`], {
                    cwd: packager.info.projectDir,
                    stdio: ["ignore", "inherit", "inherit"]
                });
            } else {
                if (options.buildPackages && options.buildPackages.length > 0) {
                    yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).spawn)("apt-get", ["update"]);
                    yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).spawn)("apt-get", ["install", "-y"].concat(options.buildPackages));
                }
                yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).spawn)("snapcraft", ["snap", "--target-arch", (0, (_core || _load_core()).toLinuxArchString)(arch), "-o", resultFile], {
                    cwd: stageDir,
