...
let getPublishConfigsForUpdateInfo = exports.getPublishConfigsForUpdateInfo = (() => {
    var _ref2 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (packager, publishConfigs, arch) {
        ...
        if (publishConfigs.length === 0) {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)("getPublishConfigsForUpdateInfo: no publishConfigs, detect using repository info");
            // https://github.com/electron-userland/electron-builder/issues/925#issuecomment-261732378
            // default publish config is github, file should be generated regardless of publish state (user can test installer locally or manage the release process manually)
            const repositoryInfo = yield packager.info.repositoryInfo;
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`getPublishConfigsForUpdateInfo: ${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).safeStringifyJson)(repositoryInfo)}`);
            if (repositoryInfo != null && repositoryInfo.type === "github") {
                ...
                if (resolvedPublishConfig != null) {
                    (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`getPublishConfigsForUpdateInfo: resolve to publish config ${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).safeStringifyJson)(resolvedPublishConfig)}`);
...
let writeUpdateInfo = (() => {
    var _ref3 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (event, _publishConfigs) {
        ...
        const sha2 = new (_electronBuilderUtil || _load_electronBuilderUtil()).Lazy(function () {
            return (0, (_asarIntegrity || _load_asarIntegrity()).hashFile)(event.file, "sha256", "hex");
        });
        const sha512 = new (_electronBuilderUtil || _load_electronBuilderUtil()).Lazy(function () {
        ...
        for (const publishConfig of publishConfigs) {
            ...
            const updateInfoFile = _path.join(outDir, `${channel}${isMac ? "-mac" : ""}.yml`);
            if (!createdFiles.has(updateInfoFile)) {
                ...
                const info = {
                    ...
                    sha512: yield sha512.value
                };
                if (packager.platform === (_core || _load_core()).Platform.WINDOWS) {
                    // backward compatibility
                    info.sha2 = yield sha2.value;
                }
                yield (0, (_fsExtraP || _load_fsExtraP()).writeFile)(updateInfoFile, (0, (_jsYaml || _load_jsYaml()).safeDump)(info));
            }
...
let getResolvedPublishConfig = (() => {
    var _ref5 = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (packager, options, arch) {
        ...
        if (!owner || !project) {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`Owner or project is not specified explicitly for ${provider}, call getInfo: owner: ${owner}, project: ${project}`);
...
class PublishManager {
    constructor(packager, publishOptions, cancellationToken) {
        ...
        if (!(0, (_electronBuilderUtil || _load_electronBuilderUtil()).isPullRequest)() || forcePublishForPr) {
            ...
            if (this.isPublish && forcePublishForPr) {
                (0, (_log || _load_log()).warn)(publishForPrWarning);
            }
    ...
    artifactCreated(event) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if ((_electronBuilderUtil || _load_electronBuilderUtil()).debug.enabled) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`artifactCreated: ${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).safeStringifyJson)(event, new Set(["packager"]))},\npublishConfigs: ${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).safeStringifyJson)(publishConfigs)},\nisPublish: ${_this.isPublish}`);
            }
            ...
            if (_this.isPublish) {
                for (const publishConfig of publishConfigs) {
                    if (_this.cancellationToken.cancelled) {
                        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`${eventFile} is not published: cancelled`);
                        break;
                    }
                    const publisher = _this.getOrCreatePublisher(publishConfig, packager.info);
                    if (publisher == null) {
                        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`${eventFile} is not published: publisher is null, ${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).safeStringifyJson)(publishConfig)}`);
                        continue;
                    }
                    if (eventFile == null) {
            ...
            if (target != null && eventFile != null && !_this.cancellationToken.cancelled) {
                if (packager.platform === (_core || _load_core()).Platform.MAC && target.name === "zip" || packager.platform === (_core || _load_core()).Platform.WINDOWS && isSuitableWindowsTarget(target) && eventFile.endsWith(".exe")) {
    ...
    addTask(promise) {
        ...
        this.publishTasks.push(promise.catch(it => {
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`Publish error: ${it.toString()}`);
            this.errors.push(it);
        }));
    }
...
function createPublisher(context, version, publishConfig, options) {
    if ((_electronBuilderUtil || _load_electronBuilderUtil()).debug.enabled) {
        (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`create publisher: ${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).safeStringifyJson)(publishConfig)}`);
    }
...
function expandPublishConfig(options, packager, arch) {
    for (const name of Object.keys(options)) {
        ...
        if (typeof value === "string") {
            const expanded = packager.expandMacro(value, arch == null ? null : (_core || _load_core()).Arch[arch]);
            if (expanded !== value) {
