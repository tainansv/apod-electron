    ...
    function processTargets(platform, types) {
        function commonArch(currentIfNotSpecified) {
            if (platform === (_core || _load_core()).Platform.MAC) {
                return args.x64 || currentIfNotSpecified ? [(_core || _load_core()).Arch.x64] : [];
            }
            const result = Array();
            if (args.x64) {
                result.push((_core || _load_core()).Arch.x64);
            }
            if (args.armv7l) {
                result.push((_core || _load_core()).Arch.armv7l);
            }
            if (args.ia32) {
                result.push((_core || _load_core()).Arch.ia32);
            }
            return result.length === 0 && currentIfNotSpecified ? [(0, (_core || _load_core()).archFromString)(process.arch)] : result;
        }
        ...
        for (const type of types) {
            ...
            if (suffixPos > 0) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).addValue)(archToType, (0, (_core || _load_core()).archFromString)(type.substring(suffixPos + 1)), type.substring(0, suffixPos));
            } else {
...
function createTargets(platforms, type, arch) {
    ...
    for (const platform of platforms) {
        const archs = platform === (_core || _load_core()).Platform.MAC ? [(_core || _load_core()).Arch.x64] : arch === "all" ? [(_core || _load_core()).Arch.x64, (_core || _load_core()).Arch.ia32] : [(0, (_core || _load_core()).archFromString)(arch == null ? process.arch : arch)];
...
function configureBuildCommand(yargs) {
    ...
    return yargs.option("mac", {
        group: buildGroup,
        alias: ["m", "o", "macos"],
        description: `Build for macOS, accepts target list (see ${(0, (_chalk || _load_chalk()).underline)("https://goo.gl/HAnnq8")}).`,
        type: "array"
    }).option("linux", {
        group: buildGroup,
        alias: "l",
        description: `Build for Linux, accepts target list (see ${(0, (_chalk || _load_chalk()).underline)("https://goo.gl/O80IL2")})`,
        type: "array"
    }).option("win", {
        group: buildGroup,
        alias: ["w", "windows"],
        description: `Build for Windows, accepts target list (see ${(0, (_chalk || _load_chalk()).underline)("https://goo.gl/dL4i8i")})`,
        type: "array"
    }).option("x64", {
        group: buildGroup,
        ...
        group: publishGroup,
        alias: "p",
        description: `Publish artifacts (to GitHub Releases), see ${(0, (_chalk || _load_chalk()).underline)("https://goo.gl/WMlr4n")}`,
