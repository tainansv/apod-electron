...
let rebuildAppNativeCode = (() => {
    var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (args) {
        ...
        const config = yield (0, (_config || _load_config()).loadConfig)(projectDir);
        ...
        yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)(process.platform === "win32" ? "node-gyp.cmd" : "node-gyp", ["rebuild"], {
            env: (0, (_yarn || _load_yarn()).getGypEnv)({ version: yield (0, (_config || _load_config()).getElectronVersion)(config, projectDir), useCustomDist: true }, args.platform, args.arch, true)
        });
    });
...
}).demandOption("publisher"), wrap(argv => (0, (_createSelfSignedCert || _load_createSelfSignedCert()).createSelfSignedCert)(argv.publisher))).help().epilog(`See the Wiki (${(0, (_chalk || _load_chalk()).underline)("https://github.com/electron-userland/electron-builder/wiki")}) for more documentation.`).strict().argv;
function wrap(task) {
    return args => {
        checkIsOutdated();
        task(args).catch((_promise || _load_promise()).printErrorAndExit);
    };
}
