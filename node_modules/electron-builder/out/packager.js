...
class Packager {
    ...
    constructor(options, cancellationToken) {
        ...
        this.tempDirManager = new (_electronBuilderUtil || _load_electronBuilderUtil()).TmpDir();
        ...
        try {
            (0, (_log || _load_log()).log)("electron-builder " + "19.5.1");
        } catch (e) {
    ...
    build() {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const config = yield (0, (_config || _load_config()).computeFinalConfig)(projectDir, configPath, devMetadata, configFromOptions);
            if ((_electronBuilderUtil || _load_electronBuilderUtil()).debug.enabled) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`Effective config: ${(0, (_electronBuilderUtil || _load_electronBuilderUtil()).safeStringifyJson)(config)}`);
            }
            yield (0, (_config || _load_config()).validateConfig)(config);
            ...
            _this.electronVersion = yield (0, (_config || _load_config()).getElectronVersion)(config, projectDir, _this.isPrepackedAppAsar ? _this.metadata : null);
            _this.muonVersion = config.muonVersion;
            _this.appInfo = new (_appInfo || _load_appInfo()).AppInfo(_this.metadata, _this);
            const cleanupTasks = [];
            const outDir = _path.resolve(_this.projectDir, (0, (_electronBuilderUtil || _load_electronBuilderUtil()).use)(config.directories, function (it) {
                return it.output;
            }) || "dist");
            return {
                outDir: outDir,
    ...
    doBuild(outDir, cleanupTasks) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            for (const _ref of _this3.options.targets) {
                ...
                let wineCheck = null;
                if (checkWine && process.platform !== "win32" && platform === (_core || _load_core()).Platform.WINDOWS) {
    ...
    installAppDependencies(platform, arch) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (options.nodeGypRebuild === true) {
                (0, (_log || _load_log()).log)(`Executing node-gyp rebuild for arch ${(_core || _load_core()).Arch[arch]}`);
                yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)(process.platform === "win32" ? "node-gyp.cmd" : "node-gyp", ["rebuild"], {
                    env: (0, (_yarn || _load_yarn()).getGypEnv)(frameworkInfo, platform.nodeName, (_core || _load_core()).Arch[arch], true)
                });
            }
            ...
            if (beforeBuild != null) {
                const performDependenciesInstallOrRebuild = yield beforeBuild({
                    ...
                    platform,
                    arch: (_core || _load_core()).Arch[arch]
                });
            ...
            if (options.npmSkipBuildFromSource !== true && platform.nodeName !== process.platform) {
                ...
                yield (0, (_yarn || _load_yarn()).installOrRebuild)(options, _this4.appDir, frameworkInfo, platform.nodeName, (_core || _load_core()).Arch[arch]);
            }
        })();
    }
    afterPack(context) {
