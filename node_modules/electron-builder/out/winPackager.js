...
class WinPackager extends (_platformPackager || _load_platformPackager()).PlatformPackager {
    constructor(info) {
        ...
        this._iconPath = new (_electronBuilderUtil || _load_electronBuilderUtil()).Lazy(() => this.getValidIconPath());
        this.computedPublisherName = new (_electronBuilderUtil || _load_electronBuilderUtil()).Lazy((0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            if (publisherName == null && cscFile != null) {
                if (process.platform === "win32") {
                    try {
                        const subject = (0, (_rfc2253Parser || _load_rfc2253Parser()).parseDn)((yield (0, (_electronBuilderUtil || _load_electronBuilderUtil()).exec)("powershell.exe", [`(Get-PfxCertificate "${cscFile}").Subject`], { timeout: 30 * 1000 }))).get("CN");
                        if (subject) {
                            return (0, (_electronBuilderUtil || _load_electronBuilderUtil()).asArray)(subject);
                        }
                    } catch (e) {
                        (0, (_log || _load_log()).warn)(`Cannot get publisher name using powershell: ${e.message}`);
                    }
                }
                try {
                    // https://github.com/digitalbazaar/forge/issues/338#issuecomment-164831585
                    const p12Asn1 = (_nodeForge || _load_nodeForge()).asn1.fromDer((yield (0, (_fsExtraP || _load_fsExtraP()).readFile)(cscFile, "binary")), false);
    ...
    signAndEditResources(file) {
        ...
        return (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* () {
            ...
            const args = [file, "--set-version-string", "FileDescription", appInfo.productName, "--set-version-string", "ProductName", appInfo.productName, "--set-version-string", "InternalName", _path.basename(appInfo.productFilename, ".exe"), "--set-version-string", "LegalCopyright", appInfo.copyright, "--set-version-string", "OriginalFilename", "", "--set-file-version", appInfo.buildVersion, "--set-product-version", appInfo.versionInWeirdWindowsForm];
            (0, (_electronBuilderUtil || _load_electronBuilderUtil()).use)(appInfo.companyName, function (it) {
