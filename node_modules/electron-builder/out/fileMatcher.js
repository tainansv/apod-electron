...
class FileMatcher {
    constructor(from, to, macroExpander, patterns) {
        ...
        this.patterns = (0, (_electronBuilderUtil || _load_electronBuilderUtil()).asArray)(patterns).map(it => this.normalizePattern(it));
    }
    normalizePattern(pattern) {
        return _path.posix.normalize(this.macroExpander(pattern));
    }
    addPattern(pattern) {
...
function createFileMatcher(info, appDir, resourcesPath, macroExpander, platformSpecificBuildOptions, buildResourceDir) {
        ...
        var _ref = (0, (_bluebirdLst || _load_bluebirdLst()).coroutine)(function* (pattern) {
            ...
            if (fromStat.isFile()) {
                ...
                if (toStat != null && toStat.isDirectory()) {
                    return yield (0, (_fs || _load_fs()).copyOrLinkFile)(pattern.from, _path.join(pattern.to, _path.basename(pattern.from)), fromStat);
                }
                yield (0, (_fsExtraP || _load_fsExtraP()).mkdirs)(_path.dirname(pattern.to));
                return yield (0, (_fs || _load_fs()).copyOrLinkFile)(pattern.from, pattern.to, fromStat);
            }
            ...
            if ((_electronBuilderUtil || _load_electronBuilderUtil()).debug.enabled) {
                (0, (_electronBuilderUtil || _load_electronBuilderUtil()).debug)(`Copying files using pattern: ${pattern}`);
            }
